...
@app.route("/ibd")
def ibd_page():
    return render_template("ibd.html")
...
def ibd_data():
    ...
    if os.path.exists(data_file):
        with open(data_file, encoding="utf-8") as f:
            return jsonify(json.load(f))
    try:
        from ibd import run_daily_fetch
        return jsonify(run_daily_fetch())
    except Exception as e:
        return jsonify({"error": str(e), "videos": []}), 500

@app.route("/ibd/refresh", methods=["POST"])
def ibd_refresh():
    try:
        from ibd import run_daily_fetch
        result = run_daily_fetch()
...
@app.route("/ibd/summarize", methods=["POST"])
def ibd_summarize():
    vid_id = request.json.get("id","") if request.is_json else ""
    if not vid_id:
        return jsonify({"error": "id required"}), 400
    ...
    if not _gemini_acquire(vid_id, "ibd"):
        return jsonify({"status": "locked", "message": "分析鎖定中，請稍後再試"}), 429
    ...
    def _run():
        ...
        try:
            ...
            if os.path.exists(DATA_FILE):
                ...
                for v in data.get("videos", []):
                    if v["id"] == vid_id:
                        ...
                        for s in result.get("stocks", []):
                            c = s.get("code","")
                            if c and _re.match(r'^[A-Z]{2,5}$', c) and c not in us:
                                us.append(c)
                        v["us_stocks"] = sorted(set(us))
                        break
                tmp = DATA_FILE + ".tmp"
            ...
            app.logger.error("IBD Gemini error %s: %s", vid_id, e)
            open(f"/tmp/gemini_err_ibd_{vid_id}", "w").close()
        finally:
            _gemini_release()
...
@app.route("/ibd/status")
def ibd_status():
    vid_id = request.args.get("id","")
    if os.path.exists(f"/tmp/gemini_done_ibd_{vid_id}"):
        return jsonify({"id": vid_id, "status": "done"})
    if os.path.exists(f"/tmp/gemini_err_ibd_{vid_id}"):
        return jsonify({"id": vid_id, "status": "error"})
    return jsonify({"id": vid_id, "status": "processing"})

@app.route("/cnbc")
def cnbc_page():
    return render_template("cnbc.html")
...
def cnbc_data():
    ...
    if os.path.exists(data_file):
        with open(data_file, encoding="utf-8") as f:
            return jsonify(json.load(f))
    try:
        from cnbc import run_daily_fetch
        return jsonify(run_daily_fetch())
    except Exception as e:
        return jsonify({"error": str(e), "videos": []}), 500

@app.route("/cnbc/refresh", methods=["POST"])
def cnbc_refresh():
    try:
        from cnbc import run_daily_fetch
        result = run_daily_fetch()
...
@app.route("/cnbc/summarize", methods=["POST"])
def cnbc_summarize():
    vid_id = request.json.get("id","") if request.is_json else ""
    if not vid_id:
        return jsonify({"error": "id required"}), 400
    ...
    if lock_info:
        return jsonify({"status": "locked",
                        "message": f"另一影片正在分析中 ({lock_info['page']})"}), 429
    if not _gemini_acquire(vid_id, "cnbc"):
        return jsonify({"status": "locked", "message": "分析鎖定中，請稍後再試"}), 429
    def _run():
        try:
            from cnbc import summarize_cnbc, DATA_FILE
            import re as _re
            result = summarize_cnbc(f"https://www.youtube.com/watch?v={vid_id}")
            if not result:
                open(f"/tmp/gemini_err_cnbc_{vid_id}", "w").close(); return
            if result.get("error"):
                if os.path.exists(DATA_FILE):
                    with open(DATA_FILE, encoding="utf-8") as f: data = json.load(f)
                    for v in data.get("videos", []):
                        if v["id"] == vid_id: v["gemini"] = result; break
                    tmp = DATA_FILE + ".tmp"
                    with open(tmp, "w", encoding="utf-8") as f:
                        json.dump(data, f, ensure_ascii=False, indent=2)
                    os.replace(tmp, DATA_FILE)
                open(f"/tmp/gemini_done_cnbc_{vid_id}", "w").close(); return
            if os.path.exists(DATA_FILE):
                with open(DATA_FILE, encoding="utf-8") as f: data = json.load(f)
                for v in data.get("videos", []):
                    if v["id"] == vid_id:
                        ...
                        v["us_stocks"] = sorted(set(us)); break
                tmp = DATA_FILE + ".tmp"
                with open(tmp, "w", encoding="utf-8") as f:
                    json.dump(data, f, ensure_ascii=False, indent=2)
                os.replace(tmp, DATA_FILE)
            open(f"/tmp/gemini_done_cnbc_{vid_id}", "w").close()
        except Exception as e:
            app.logger.error("CNBC Gemini %s: %s", vid_id, e)
            open(f"/tmp/gemini_err_cnbc_{vid_id}", "w").close()
        finally:
            _gemini_release()
    threading.Thread(target=_run, daemon=True).start()
    return jsonify({"status": "processing"})

@app.route("/cnbc/status")
def cnbc_status():
    vid_id = request.args.get("id","")
    if os.path.exists(f"/tmp/gemini_done_cnbc_{vid_id}"):
        return jsonify({"id": vid_id, "status": "done"})
    if os.path.exists(f"/tmp/gemini_err_cnbc_{vid_id}"):
        return jsonify({"id": vid_id, "status": "error"})
    return jsonify({"id": vid_id, "status": "processing"})
